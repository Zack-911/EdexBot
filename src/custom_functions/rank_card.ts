...
export default new ForgeFunction({
  ...
  code: `
    $return[$djsEval[(async () => {
      ...
      ctx.fillText("$username[$env[user;id]]", contentX, usernameY);
      ...
      ctx.fillText("LEVEL $env[level]", contentX, usernameY + lineGap);
    
      // ---- XP bar ----
      const barHeight = height*0.055;
      const barY = usernameY + lineGap*1.8;
      const xp = Number("$env[xp]");
      const req = Number("$env[req]");
